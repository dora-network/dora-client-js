/*
 * DORA
 * No description provided (generated by Swagger Codegen https://github.com/swagger-api/swagger-codegen)
 *
 * OpenAPI spec version: 0.1.0
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 *
 * Swagger Codegen version: 3.0.69
 *
 * Do not edit the class manually.
 *
 */
import ApiClient from '../ApiClient';

/**
 * The OrderBookSummary model module.
 * @module model/OrderBookSummary
 * @version 0.1.0
 */
export default class OrderBookSummary {
  /**
   * Constructs a new <code>OrderBookSummary</code>.
   * @alias module:model/OrderBookSummary
   * @class
   */
  constructor() {
  }

  /**
   * Constructs a <code>OrderBookSummary</code> from a plain JavaScript object, optionally creating a new instance.
   * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
   * @param {Object} data The plain JavaScript object bearing properties of interest.
   * @param {module:model/OrderBookSummary} obj Optional instance to populate.
   * @return {module:model/OrderBookSummary} The populated <code>OrderBookSummary</code> instance.
   */
  static constructFromObject(data, obj) {
    if (data) {
      obj = obj || new OrderBookSummary();
      if (data.hasOwnProperty('order_book_id'))
        obj.orderBookId = ApiClient.convertToType(data['order_book_id'], 'String');
      if (data.hasOwnProperty('mid_price'))
        obj.midPrice = ApiClient.convertToType(data['mid_price'], 'String');
      if (data.hasOwnProperty('spread'))
        obj.spread = ApiClient.convertToType(data['spread'], 'String');
      if (data.hasOwnProperty('best_bid'))
        obj.bestBid = ApiClient.convertToType(data['best_bid'], 'String');
      if (data.hasOwnProperty('best_bid_quantity'))
        obj.bestBidQuantity = ApiClient.convertToType(data['best_bid_quantity'], 'String');
      if (data.hasOwnProperty('best_ask'))
        obj.bestAsk = ApiClient.convertToType(data['best_ask'], 'String');
      if (data.hasOwnProperty('best_ask_quantity'))
        obj.bestAskQuantity = ApiClient.convertToType(data['best_ask_quantity'], 'String');
      if (data.hasOwnProperty('timestamp'))
        obj.timestamp = ApiClient.convertToType(data['timestamp'], 'Date');
    }
    return obj;
  }
}

/**
 * @member {String} orderBookId
 */
OrderBookSummary.prototype.orderBookId = undefined;

/**
 * @member {String} midPrice
 */
OrderBookSummary.prototype.midPrice = undefined;

/**
 * @member {String} spread
 */
OrderBookSummary.prototype.spread = undefined;

/**
 * @member {String} bestBid
 */
OrderBookSummary.prototype.bestBid = undefined;

/**
 * @member {String} bestBidQuantity
 */
OrderBookSummary.prototype.bestBidQuantity = undefined;

/**
 * @member {String} bestAsk
 */
OrderBookSummary.prototype.bestAsk = undefined;

/**
 * @member {String} bestAskQuantity
 */
OrderBookSummary.prototype.bestAskQuantity = undefined;

/**
 * @member {Date} timestamp
 */
OrderBookSummary.prototype.timestamp = undefined;

